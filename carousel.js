class Carousel {
     ...
     * @returns 
     */
    constructor(carouselId, itemsClassNames, carouselNavId = false, leftButtonId = false, rightButtonId = false, autoChange = true, autoChangeDelay = 5000, loop = true, mobileTouch = true) {
        if (!carouselId) {
    ...
    init() {
        ...
        if (this.carouselNav) {
            this.carouselNav.querySelectorAll('.carousel-nav-point')[0].classList.add('active');
        }
        ...
        if (this.mobileTouch) {
            this.carousel.addEventListener('touchstart', this.touchStart.bind(this), false);
            this.carousel.addEventListener('touchmove', this.touchMove.bind(this), false);
            this.carousel.addEventListener('touchend', this.touchEnd.bind(this), false);
        }
    }
    ...
    slideLeft(last = false) {
        if (last) {
            this.items.forEach(item => {
                const end = this.itemWidth * (this.items.length - 1) + this.carouselGap * (this.items.length - 1);
                for (let i = 0; i < end; i++) {
                    setTimeout(() => {
            ...
            this.items.forEach(item => {
                ...
                item.style.transform = `translateX(${actualTransform + this.itemWidth + this.carouselGap}px)`;
            });
        }
    }
    ...
    updateNav() {
        ...
        this.carouselNav.querySelectorAll('.carousel-nav-point')[activeIndex].classList.add('active');
    }
    ...
    updateArrows() {
        ...
        if (this.carousel.querySelector('.active').previousElementSibling === null) {
            if (this.leftButton) {
